...
const Films: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2 py-10 px-4 md:gap-4 md:py-20 md:px-10">
      {films.length < 1 ? (
        <FilmListSkeleton />
      ) : (
        <>
            ...
            <div key={film.episode_id} className="min-h-[150px] md:min-h-[200px]">
              <div className="w-full relative h-full">
                ...
                <div className="z-20 absolute bottom-0 left-0 z-10 ms-2 mb-2 md:-translate-y-2/4 md:ms-4 flex flex-col md:flex-row gap-2">
                  <h2 className="text-lg md:text-xl font-bold order-2 md:order-1">{film.title}</h2>
                  {index === 2 && (
